...
/**
 * Controller class for the User Verification Window interface.
 * This class handles the initial password verification step for sensitive operations,
 * particularly user account deletion. It serves as the first security layer by
 * requiring the user to confirm their identity with their password before proceeding
 * to additional verification steps.
 * 
 * The controller implements JavaFX Initializable interface to properly
 * initialize the UI components and manage the user verification workflow.
 *
...
public class VerifyUserWindowController implements Initializable {
     ...
     * This method validates the user's password against the stored profile password,
     * and if successful, navigates to the next verification step (action confirmation window).
     * It transfers the callback function to the next verification step and displays
     * appropriate error messages for invalid or missing passwords.
     *
     * @throws IOException if there is an error loading the action verification
     *         window FXML file or if the resource cannot be found
     */
    @FXML
     ...
     * This method closes the user verification window without performing any
