...
/**
 * Main controller class that serves as the central coordinator between the user interface
 * and the data access layer. This class handles application initialization, window management,
 * and delegates business operations to the underlying DAO implementation.
 * 
 * The controller follows the Facade pattern by providing a simplified interface
 * to complex subsystem operations while managing the application's main workflow
 * and user navigation.
 *
...
public class Controller
{
    ...
    /**
     * Constructs a new Controller instance and initializes the data access layer.
     * This constructor attempts to establish a connection to the database through
     * the DBImplementation class. If the database connection fails, an exception
     * is thrown with a descriptive error message.
     *
     * @throws OurException if the database connection cannot be established,
     *         containing details about the connection failure
     */
    ...
    /**
     * Displays the main application window starting with the login interface.
     * This method initializes the primary stage with the login window scene,
     * configures the window properties including title and icon, and sets up
     * the controller reference for the login interface.
     *
     * @param stage the primary stage to be used for displaying the application window
     * @throws IOException if the FXML file for the login window cannot be loaded
     *         or if there are issues with the resource loading process
     */
     ...
     * This method delegates the user registration process to the data access layer,
     * which handles the persistence of user data and ensures data integrity.
     *
     * @param user the User object containing all registration information
     * @return the registered User object, typically with generated identifiers
     *         and any system-assigned values
     * @throws OurException if the registration process fails due to validation errors,
     *         duplicate users, or database constraints violations
     */
     ...
     * @throws OurException if authentication fails due to invalid credentials,
     *         user not found, or database access issues
     */
